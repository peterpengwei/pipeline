...
void gather(void) {
	...
	start_time = tic();
        if (instance < 0) {
            ...
            char* buffer = (char *)clEnqueueMapBuffer(commands, contents[buf_ptr], CL_TRUE, CL_MAP_WRITE, 0, TILE_SIZE, 0, NULL, NULL, NULL);
	    ...
	    close(instance);
            clEnqueueUnmapMemObject(commands, contents[buf_ptr], buffer, 0, NULL, NULL);
...
void compute(void) {
	...
	start_time = tic();
        ...
        //
        err  = clSetKernelArg(kernel, 0, sizeof(cl_mem), &key_buf);
        err |= clSetKernelArg(kernel, 1, sizeof(cl_mem), &(contents[cur_idx]));
        err |= clSetKernelArg(kernel, 2, sizeof(int), &TILE_SIZE);
        err |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &(results[cur_idx]));
        if (err != CL_SUCCESS)
        {
...
void scatter(void) {
	...
	int cur_idx;
        ...
        char* buffer = (char *)clEnqueueMapBuffer(commands, results[cur_idx], CL_TRUE, CL_MAP_READ, 0, sizeof(int), 0, NULL, NULL, NULL);
        int sock;
	...
	int n;
        while ((n = write(sock, buffer, total_size)) > 0) {
	    if (n >= total_size) break;
	    p += n;
	    total_size -= n;
	}
	close(sock);
        clEnqueueUnmapMemObject(commands, results[cur_idx], buffer, 0, NULL, NULL);
...
int main(int argc, char* argv[]) {
    ...
    key_buf = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(key), NULL, NULL);
    err = clEnqueueWriteBuffer(commands, key_buf, CL_TRUE, 0, sizeof(key), key, 0, NULL, NULL);
    if (err != CL_SUCCESS)
    {
    ...
    for (i=0; i<NUM_BUFFERS; i++) {
      ...
      //
      contents[i] = clCreateBuffer(context, CL_MEM_READ_WRITE, TILE_SIZE, NULL, NULL);
      if (!contents[i])
      {
      ...
      results[i] = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(int), NULL, NULL);
      if (!results[i])
      {
        printf("Error: Failed to allocate device memory!\n");
        printf("Test failed\n");
