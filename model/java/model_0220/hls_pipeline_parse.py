...
while size_idx <= int(sys.argv[2]):
    ...
    for j in [1, 0]:
        for k in [1]:
            ...
            size_str = str(size / 1024) + 'KiB_' + str(j) + '_' + str(k)
            if j == 1:
...
clload = [[], []]
clload[0].append("Size")
clload[1].append("Latency")
for i in xrange(int(sys.argv[1]), int(sys.argv[2])+1):
    payload_size = (1 << i)
    basic_idx = i - int(sys.argv[1]) + 1
    num_iter = 128
    ...
    assert len(numbers) == 10
    for num in numbers:
        clload[0].append(str(payload_size))
        clload[1].append(str(num / num_iter))
...
output = open("clload.csv", 'w')
for numbers in clload:
